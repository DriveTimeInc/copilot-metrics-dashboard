...
export const applyTimeFrameLabel = (
  ...
  sortedData.forEach(item => {
    ...
    const output: CopilotUsageOutput = {
      ...
      total_ide_engaged_users:
        item.copilot_ide_code_completions.total_engaged_users,
      total_code_suggestions: (
        ...
        (acc, editor) =>
          ...
          editor.models.reduce(
            (modelAcc, model) =>
              modelAcc +
              model.languages!.reduce(
                (langAcc, lang) => langAcc + (lang.total_code_suggestions || 0),
                0,
              ),
            0,
          ),
        ...
        (acc, editor) =>
          ...
          editor.models.reduce(
            (modelAcc, model) =>
              modelAcc +
              model.languages!.reduce(
                (langAcc, lang) => langAcc + (lang.total_code_acceptances || 0),
                0,
              ),
            0,
          ),
        ...
        (acc, editor) =>
          ...
          editor.models.reduce(
            (modelAcc, model) =>
              ...
              model.languages!.reduce(
                (langAcc, lang) =>
                  langAcc + (lang.total_code_lines_suggested || 0),
                0,
              ),
            0,
          ),
        ...
        (acc, editor) =>
          ...
          editor.models.reduce(
            (modelAcc, model) =>
              ...
              model.languages!.reduce(
                (langAcc, lang) =>
                  langAcc + (lang.total_code_lines_accepted || 0),
                0,
              ),
            0,
          ),
      ...
      total_chat_insertion_events: (item.copilot_ide_chat.editors || []).reduce(
        (acc, editor) =>
          ...
          editor.models.reduce(
            (modelAcc, model) =>
              modelAcc + (model.total_chat_insertion_events || 0),
            0,
          ),
        0,
      ),
...
export const getNextUrlFromLinkHeader = (
